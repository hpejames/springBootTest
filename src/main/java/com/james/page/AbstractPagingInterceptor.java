    ...
    private void getTotalAndSetInPagingBounds(String targetSql, BoundSql boundSql, PagingBounds<?> bounds, 
        ...
        try {
            ...
            if(rs.next()) {
                ...
                int totalPage = totalRecord / bounds.getPageSize();
                if (totalRecord % bounds.getPageSize() != 0) {
                    totalPage++;
                }
                bounds.setTotalPage(totalPage);
                if (bounds.getPageNo() > totalPage) {
                	bounds.setPageNo(totalPage);
                }
            }
        } finally {
            if(rs != null) {
